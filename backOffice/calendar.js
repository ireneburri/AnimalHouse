$(() => {
    ...
    $('#min-date').dxCheckBox({
      ...
      onValueChanged(data) {
        if (data.value) {
          calendar.option('min', new Date().getTime());
        } else {
    ...
    $('#disable-someDates').dxCheckBox({
        text: 'Disable some dates',
        onValueChanged(data) { 
          if (data.value) {
            calendar.option('disabledDates', (d) => d.view === 'month' && d.getDate === new Date('December 25, 2022 23:15:30'));
          } else {
            calendar.option('disabledDates', null);
          }
        },
